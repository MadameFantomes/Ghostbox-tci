...
/**
 * Ghostbox TCI — FR (vintage + sliders + MP3 via CDN + MODE KÖNIG)
 ...
 * - MODE KÖNIG : ring-mod démultiplié + LPF, multi-porteuses battements, skip musique auto
...
export default function Page() {
  ...
  const [kng, setKng] = useState(false); // <<< MODE KÖNIG
  ...
  const [debit] = useState(0.35); // burst color
  ...
  const cacheBust = (u) => { try { const url = new URL(u); url.searchParams.set("gbx", Date.now().toString(36)); return url.toString(); } catch { return u + (u.includes("?") ? "&" : "?") + "gbx=" + Date.now().toString(36); } };
  async function hardStop(el) { if (!el) return; try { el.pause(); } catch {} try { el.removeAttribute("src"); } catch {} try { el.src = ""; } catch {} try { el.load(); } catch {} await sleep(40); }
  function waitFor(el, events = ["playing", "canplay"], timeout = 2500) {
    return new Promise((resolve, reject) => {
      let done = false; const on = () => { if (done) return; done = true; cleanup(); resolve(); };
  ...
  function addLog(event, extra = {}) {
    const entry = { timestamp: new Date().toISOString(), event, vitesse, lent, kng, volume, echo, station: stationsRef.current[idxRef.current]?.name || null, ...extra };
    logRef.current.push(entry);
    try { const key = "ghostbox.logs"; const prev = JSON.parse(localStorage.getItem(key) || "[]"); prev.push(entry); localStorage.setItem(key, JSON.stringify(prev)); } catch {}
  ...
  useEffect(() => { (async () => {
    try { const r = await fetch("/stations.json", { cache: "no-store" }); if (!r.ok) throw new Error();
  ...
  function normalizeStationsJson(json) {
    let list = []; const push = (name, url, suffix = "") => {
      if (!url || typeof url !== "string") return; if (!/^https:/i.test(url)) return;
      try { url = url.trim(); list.push({ name: (name && name.trim()) || safeHost(url) + suffix, url }); } catch {}
    };
    if (Array.isArray(json)) { json.forEach((s) => { if (!s) return; if (s.url) push(s.name, s.url); if (Array.isArray(s.urls)) s.urls.forEach((u, k) => push(s.name, u, ` #${k + 1}`)); }); }
    else if (json && typeof json === "object") { Object.entries(json).forEach(([group, arr]) => Array.isArray(arr) && arr.forEach((s) => {
      if (!s) return; if (s.url) push(s.name || group, s.url); if (Array.isArray(s.urls)) s.urls.forEach((u, k) => push(s.name || group, u, ` #${k + 1}`));
    })); }
    const seen = new Set(); list = list.filter((s) => (seen.has(s.url) ? false : seen.add(s.url)));
  ...
  function createNoiseNode(ctx) { const size = 2 * ctx.sampleRate; const buf = ctx.createBuffer(1, size, ctx.sampleRate); const data = buf.getChannelData(0); for (let i = 0; i < size; i++) data[i] = (Math.random() * 2 - 1) * 0.9; const src = ctx.createBufferSource(); src.buffer = buf; src.loop = true; return src; }
  function createDriveNode(ctx, amount = 0.25) { const ws = ctx.createWaveShaper(); ws.curve = makeDistortionCurve(amount); ws.oversample = "4x"; return ws; }
  function makeDistortionCurve(amount = 0.25) { const k = amount * 100, n = 44100, curve = new Float32Array(n); for (let i = 0; i < n; i++) { const x = (i * 2) / n - 1; curve[i] = ((1 + k) * x) / (1 + k * Math.abs(x)); } return curve; }
  function applyAutoFilterProfile() {
    ...
    try { radioHPRef.current.frequency.setTargetAtTime(hpF, now, 0.08); radioLPRef.current.frequency.setTargetAtTime(lpF, now, 0.08); radioShelfRef.current.gain.setTargetAtTime(shelfGain, now, 0.1); driveRef.current.curve = makeDistortionCurve(driveAmt); } catch {}
  }
  ...
  async function attachMedia() {
    ...
    try {
      ...
      const duckToFloor = () => { const now = ctxRef.current.currentTime;
      ...
      el.addEventListener("playing", duckToFloor); el.addEventListener("timeupdate", duckToFloor);
  ...
  function kngEnable() {
    ...
    // 2) Multi-porteuses + battements (faible niveau)
    const bus = ctx.createGain(); bus.gain.value = 0.18; // niveau global porteuses
    ...
    freqs.forEach((f) => {
      ...
      const g = ctx.createGain(); g.gain.value = 0.006; // base amplitude très faible
      ...
      const lfoAmp = ctx.createGain(); lfoAmp.gain.value = 0.004; // profondeur
      lfo.connect(lfoAmp); lfoAmp.connect(g.gain);
      // Offset DC pour part d'amplitude constante
      const base = ctx.createConstantSource(); base.offset.value = 0.004; base.connect(g.gain); base.start();
    ...
    // monter doucement le mix traité
    try { wet.gain.setTargetAtTime(0.35, ctx.currentTime, 0.15); } catch {}
  ...
  useEffect(() => { if (!ctxRef.current) return;
    if (kng) kngEnable(); else kngDisable();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [kng]);

  // ------- Power -------
  async function powerOn() {
    await initAudio(); const ctx = ctxRef.current; if (ctx.state === "suspended") await ctx.resume();
    try { noiseGainRef.current.gain.value = BASE_NOISE; } catch {}
    addLog("power_on"); await playIndex(idxRef.current); setMarche(true);
  }
  async function powerOff() {
    stopSweep(); const el = audioElRef.current;
  ...
  function triggerClick(level = 0.28) {
    ...
    const dur = 0.012; const buf = ctx.createBuffer(1, Math.max(1, Math.floor(dur * ctx.sampleRate)), ctx.sampleRate);
    const d = buf.getChannelData(0); for (let i = 0; i < d.length; i++) d[i] = (Math.random() * 2 - 1) * Math.pow(1 - i / d.length, 12);
    const src = ctx.createBufferSource(); src.buffer = buf; const g = clickBusRef.current; const now = ctx.currentTime;
  ...
  function playScanBurst(targetGain, durSec) {
    ...
    const fStart = 800 + Math.random() * 800, fMid = 1300 + Math.random() * 1500, fEnd = 1800 + Math.random() * 1800; const steps = Math.max(4, Math.floor(durSec * 10));
  ...
  async function kngEvaluateStation(msWindow = 700, frames = 10) {
    const an = kngAnalyserRef.current; if (!kng || !an) return { keep: true, score: 0 };
    const arr = new Float32Array(an.frequencyBinCount);
    let sumFlat = 0;
    for (let i = 0; i < frames; i++) {
      an.getFloatFrequencyData(arr);
      // convert dB -> lin amplitude
    ...
    // Heuristique : <0.32 très tonal (musique) -> skip
  ...
  async function playIndex(startIndex, tries = 0) {
    ...
    try { if (!compat) { radioGainRef.current.gain.cancelScheduledValues(now); radioGainRef.current.gain.linearRampToValueAtTime(0.0001, now + 0.05); } else { el.volume = 0; } } catch {}
    playScanBurst(targetNoise, dur); setEtat("balayage…"); addLog("scan_burst", { targetNoise, dur });
    await sleep(Math.max(80, dur * 500));

    // 2) HARD STOP + cache-busting + lecture
    try {
      await hardStop(el);
      el.crossOrigin = "anonymous"; el.preload = "none"; el.src = cacheBust(url);
      try { el.playbackRate = lent ? 0.88 : 1; if ("preservesPitch" in el) el.preservesPitch = false; if ("mozPreservesPitch" in el) el.mozPreservesPitch = false; if ("webkitPreservesPitch" in el) el.webkitPreservesPitch = false; } catch {}
      const playP = el.play().catch(() => {}); await Promise.race([waitFor(el, ["playing", "canplay"], 2200), playP]);

      await attachMedia(); applyAutoFilterProfile();

      // 2b) MODE KÖNIG : rapidement analyser la station et skipper si trop musicale
      if (kng && auto) {
        const ev = await kngEvaluateStation(650, 8);
        addLog("kng_eval", { flatness: Number(ev.score.toFixed(3)), keep: ev.keep });
        if (!ev.keep) {
      ...
      try {
        ...
        if (!compat) radioGainRef.current.gain.setTargetAtTime(1, after, 0.07); else el.volume = clamp01(volume);
  ...
  async function loadLame() {
    ...
    const cdns = ["https://cdn.jsdelivr.net/npm/lamejs@1.2.1/lame.min.js", "https://unpkg.com/lamejs@1.2.1/lame.min.js"];
    for (const url of cdns) { try { await loadScript(url); break; } catch {} }
    const lib = window.lamejs || window.Lame || window.lame; if (!lib) throw new Error("lamejs introuvable"); return lib;
  }
  ...
  async function toggleEnr() {
    ...
    if (!enr) {
      ...
      if (useNative) {
        let rec; try { rec = new MediaRecorder(destRef.current.stream, { mimeType: "audio/mpeg" }); } catch { rec = new MediaRecorder(destRef.current.stream); }
        chunksRef.current = []; rec.ondataavailable = (e) => { if (e.data && e.data.size) chunksRef.current.push(e.data); };
        rec.onstop = () => { const blob = new Blob(chunksRef.current, { type: rec.mimeType || "audio/mpeg" }); const url = URL.createObjectURL(blob); const a = document.createElement("a"); a.href = url; a.download = `ghostbox-${ts()}.mp3`; document.body.appendChild(a); a.click(); document.body.removeChild(a); URL.revokeObjectURL(url); };
        ...
        try {
          ...
          let rec; try { rec = new MediaRecorder(destRef.current.stream, { mimeType: "audio/webm;codecs=opus" }); } catch { rec = new MediaRecorder(destRef.current.stream); }
          chunksRef.current = []; rec.ondataavailable = (ev) => { if (ev.data && ev.data.size) chunksRef.current.push(ev.data); };
          rec.onstop = () => { const blob = new Blob(chunksRef.current, { type: "audio/webm" }); const url = URL.createObjectURL(blob); const a = document.createElement("a"); a.href = url; a.download = `ghostbox-${ts()}.webm`; document.body.appendChild(a); a.click(); document.body.removeChild(a); URL.revokeObjectURL(url); };
      ...
      if (tapProcRef.current && mp3EncoderRef.current) {
        try {
          ...
          const blob = new Blob(mp3ChunksRef.current, { type: "audio/mpeg" }); const url = URL.createObjectURL(blob);
          const a = document.createElement("a"); a.href = url; a.download = `ghostbox-${ts()}.mp3`; document.body.appendChild(a); a.click(); document.body.removeChild(a); URL.revokeObjectURL(url);
        } catch {}
  ...
  function f32ToI16(f32) { const out = new Int16Array(f32.length); for (let i = 0; i < f32.length; i++) { const s = Math.max(-1, Math.min(1, f32[i])); out[i] = s < 0 ? s * 0x8000 : s * 0x7fff; } return out; }
  function ts() { return new Date().toISOString().replace(/[:.]/g, "-"); }

  /* ------- Réactions contrôles ------- */
  useEffect(() => { if (masterRef.current) masterRef.current.gain.value = clamp01(volume); if (audioElRef.current && compat) audioElRef.current.volume = clamp01(volume); }, [volume, compat]);
  useEffect(() => { if (!echoWetRef.current || !echoFbRef.current) return; echoWetRef.current.gain.value = echo * 0.9; echoFbRef.current.gain.value = Math.min(0.6, echo * 0.6); }, [echo]);
  useEffect(() => { if (!auto) { stopSweep(); return; } if (marche) startSweep(); return stopSweep; }, [auto, vitesse, marche, lent]);
  useEffect(() => { const el = audioElRef.current; if (!el) return; try { el.playbackRate = lent ? 0.88 : 1; if ("preservesPitch" in el) el.preservesPitch = false; if ("mozPreservesPitch" in el) el.mozPreservesPitch = false; if ("webkitPreservesPitch" in el) el.webkitPreservesPitch = false; } catch {} }, [lent]);
  ...
  return (
    <main style={styles.page}>
      ...
      <p style={{ color: "rgba(60,40,20,0.85)", marginTop: 10, fontSize: 12 }}>
        Mode KÖNIG : ring-mod + battements + skip musique. Enreg. MP3 (CDN) OK.
      </p>
    </main>
  );
}
...
function Slider({ label, value, onChange, hint }) {
  ...
  const setFromClientX = (clientX) => { const rect = barRef.current.getBoundingClientRect(); const x = (clientX - rect.left) / rect.width; onChange(Math.max(0, Math.min(1, x))); };
  return (
    <div style={ui.sliderBlock}>
      <div style={ui.sliderLabel}>{label}</div>
      <div ref={barRef} style={ui.sliderRail}
...
function Bouton({ label, onClick, color = "#b77d56" }) { return <button onClick={onClick} style={{ ...ui.button, background: color }}>{label}</button>; }
...
function Vis({ at }) { const pos = { tl: { top: -8, left: -8 }, tr: { top: -8, right: -8 }, bl: { bottom: -8, left: -8 }, br: { bottom: -8, right: -8 } }[at] || {}; return <div style={{ ...decor.screw, ...pos }} />; }
...
const ui = {
  ...
  sliderRail: { position: "relative", width: "100%", height: 30, borderRadius: 16, border: "1px solid rgba(70,50,30,0.6)", background: "linear-gradient(180deg,#d7c2a4,#b79a77)", boxShadow: "inset 0 2px 0 rgba(255,255,255,0.6), inset 0 -3px 0 rgba(0,0,0,0.15)", touchAction: "none", userSelect: "none", cursor: "pointer" },
  sliderFill: { position: "absolute", left: 0, top: 0, bottom: 0, borderRadius: 16, background: "linear-gradient(180deg,#b98556,#9a6b42)" },
  sliderThumb: { position: "absolute", top: "50%", transform: "translate(-50%,-50%)", width: 22, height: 22, borderRadius: "50%", background: "#6e553d", border: "1px solid rgba(40,25,10,0.5)", boxShadow: "0 3px 6px rgba(0,0,0,0.3), inset 0 1px 0 rgba(255,255,255,0.4)" },
...
const decor = { screw: { position: "absolute", width: 18, height: 18, borderRadius: "50%", background: "radial-gradient(circle at 30% 30%, #b6b0a7, #6c655c 60%, #3b362f)", border: "1px solid #2a2621", boxShadow: "0 8px 20px rgba(0,0,0,0.5), inset 0 3px 8px rgba(0,0,0,0.6)" } };
